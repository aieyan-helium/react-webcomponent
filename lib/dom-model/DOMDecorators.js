...
function queryChildren(element, selector, all = false) {
    ...
    let result = all
        ...
        : element.querySelector(scopedSelector);
    if (!id) {
...
function attachContentObserver(target, key, element, child, valueFn) {
    if (child && !child._isObserved) {
        ...
        observer.observe(child.childNodes[0], {
            characterData: true,
            childList: false,
        });
                ...
                let valueFn = function (domElement, childrenMap) {
                    ...
                    if (domElement && domElement instanceof HTMLElement) {
                        ...
                        for (let i = 0; i < children.length; ++i) {
                            ...
                            let refType =
                                childrenMap[child.nodeName.toLowerCase()];
                            if (refType) {
                                let value = new refType();
                                value.fromDOM(child);
                                result.push(value);
            ...
            target.addProperty(key, (element) => {
                if (element && element instanceof HTMLElement) {
                    ...
                    if (
                        ...
                        typeof child._generateModel === "function"
                    ) {
