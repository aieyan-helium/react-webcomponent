...
describe("WebComponent", () => {
                    ...
                    <table>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td id="jAttr">
                                    {JSON.stringify(this.props.jAttr)}
                                </td>
                            </tr>
                            <tr>
                                <td> value </td>
    ...
    window.customElements.define(
        "custom-component",
        createCustomElement(ReactComponent, Model, "container")
    );
    ...
    it("should have all the attributes", (done) => {
        customElements.whenDefined("custom-component").then(() => {
            ...
            requestAnimationFrame(() => {
                expect(element.querySelector("#weight").textContent).to.equal(
                    "3"
                );
                expect(element.querySelector("#value").textContent).to.equal(
                    "Test content"
                );
                expect(element.querySelector("#jAttr").textContent).to.equal(
                    '[{"example":1},{"test":2}]'
                );
    ...
    it("default value should be respected", (done) => {
        customElements.whenDefined("custom-component").then(() => {
            document.body.appendChild(element);
            requestAnimationFrame(() => {
                expect(element.querySelector("#height").textContent).to.equal(
                    "5"
                );
                done();
            });
    ...
    it("attributes should update", (done) => {
        customElements.whenDefined("custom-component").then(() => {
            ...
            requestAnimationFrame(() => {
                ...
                requestAnimationFrame(() => {
                    expect(
                        element.querySelector("#weight").textContent
                    ).to.equal("5");
                    ...
                    requestAnimationFrame(() => {
                        expect(
                            element.querySelector("#jAttr").textContent
                        ).to.equal('[{"example":3},{"test":4}]');
                        element.querySelector("child-value").textContent =
                            "SOMETHING";
                        requestAnimationFrame(() => {
                            expect(
